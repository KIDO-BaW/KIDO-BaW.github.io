...
if (canvas && hero) {
  ...
  function resetStar(star, z) {
    Object.assign(star, createStar(z));
  }
  ...
  function createStars() {
    stars.length = 0;
    const count = width < 560 ? 430 : width < 900 ? 680 : 980;

    for (let index = 0; index < count; index += 1) {
      stars.push(createStar(randomBetween(0.08, 1)));
    }
  }
  ...
  function animate(timeMs) {
    ...
    if (timeMs - lastFrameTime < 1000 / 48) {
      requestAnimationFrame(animate);
      return;
    }
    ...
    for (let index = 0; index < stars.length; index += 1) {
      drawStar(stars[index], time, delta);
    }
    drawSparks();
